...
Sprinkle these throughout your code to see how much RAM you have available at different locations and identify the steps that are using the most RAM. Be sure to included `gc.collect()` prior to printing the amount of memory free. We’ll explain later why. 
...
Sometimes, you can see with gc.mem_free() that you have plenty of memory available, but you still get a message about “Memory allocation failed”. Ok now we get deeper into how CircuitPython manages memory with the garbage collector. 
...
- Video: Project specific - @foamyguy did a livestream on saving memory on the EZbake oven specifically for the PyPortal Titano: https://youtu.be/lDAyfZp_350
